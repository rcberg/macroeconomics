...
function build_model(fns; 
    ...
    # incorporate the compiled cost/expenditure functions, including gradients and jacobians
    # note how the jacobian is supplied as the hessian for the optimization algorithm.
    ...
    elseif isnothing(u_bar)
        ...
        @variable(urban_economy, 0.0 <= η_x <= 1.0, start = 0.6949 )
        @variable(urban_economy, 0.0 <= γ_L <= 1.0, start = 0.0134 )
        ...
        @variable( urban_economy, N_total >= 0.0, start = 1.0)
